...
import { useSelector, useDispatch, selectCart, cartSlice, selectProducts } from '@/lib/redux'
...
export const Products = () => {
  ...
  const handleAddToCart = (product: Product, cart: { cartTotalPrice: number; }) => {
    dispatch(cartSlice.actions.addToCart(product.productName))
    dispatch(cartSlice.actions.setTotalPriceState(cart.cartTotalPrice + product.productData.price));
  }
            ...
            <div className={styles.container}>
              {productsArr?.map(({ productName, productData }) => (
                <ProductCard 
                  ...
                  handleClick={() => handleAddToCart({ productName, productData }, cart)}
