...
export const ShoppingCartItem: React.FC<ShoppingCartItemProps> = ({
  ...
  const handleClickDecrement = (
    ...
    dispatch(
      cartSlice.actions.setTotalPriceState(
        cart.cartTotalPrice - productData.price
      )
    );
  };
  ...
  const handleClickDelete = (
    ...
    dispatch(
      cartSlice.actions.setTotalPriceState(
        cart.cartTotalPrice - productData.price * orderedAmount
      )
    );
  };
  ...
  return (
    <div className={styles.cartItem}>
      ...
      <div className={styles.cartItemControls}>
        <CustomButton
          title="-"
          isDisabled={orderedAmount === 1}
          classNames={styles.buttonMinus}
          handleClick={() => handleClickDecrement(productName, productData)}
        />
        <div>{orderedAmount}</div>
        <CustomButton
          title="+"
          isDisabled={orderedAmount === productData.quantity}
          classNames={styles.buttonPlus}
          handleClick={() => handleClickIncrement(productName, productData)}
        />
        <button
          type="button"
          className={styles.delete}
          onClick={() =>
            handleClickDelete(productName, productData, orderedAmount)
          }
        >
          x
        </button>
